...
// Comented by Rachel Saya for CS 3500, October 2015
// Version 1.7
// Revision history:  
...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        protected IEnumerable<String> GetCellsToRecalculate(ISet<String> names)
        {
            ...
            {
                if (!visited.Contains(name))                            //If name has been visited
                {
                    Visit(name, name, visited, changed);                //Call recursive visit function
                }
            }
            return changed;
        }
        ...
        {
            visited.Add(name);
            foreach (String n in GetDirectDependents(name))             //For every direct dependent of name
            {
                if (n.Equals(start))                                    //If that dependent is the same as start                          
                {
                    throw new CircularException();                      //Throw excpetion to avoid infinite
                }
                else if (!visited.Contains(n))                          //If it has not been visited before
                {
                    Visit(start, n, visited, changed);                  //Recursive call
                }
            }
