...
// Revised by Nate Merrill and Rachel Saya for CS 3500, November 2015
//
// Revision history:  
//   Version 1.1 11/03/15 10:05 a.m.   Added key scrolling capabilities
//   Version 1.2 11/04/15 12:37 p.m.   Added highlighting capabilities
...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        /// <summary>
        /// Sets the cells that have been updated because of user input
        /// </summary>
        /// <param name="args"></param>
        public void SetUpdatedCells(Dictionary<string, string> args)
        {
            drawingPanel.SetUpdatedCells(args);
            return;
        ...
        private class DrawingPanel : Panel
        {
            ...
            // Dictionary of cells that have been changed as a result of a formula/input
            private Dictionary<string, string> cellsChanged;
            ...
            public bool SetSelection(int col, int row)
            {
                ...
                // If the currently selected row is less than the first row or greater than the maximum row
                if (_selectedRow > maximumRow() || _selectedRow < _firstRow)
                {
                    _firstRow = _selectedRow;
                }

                // If the currently selected col is less than the first col or greater than the maximum col
                if ((_selectedCol > maximumCol()) || _selectedCol < _firstColumn)
                {
                    _firstColumn = _selectedCol;
                }
            ...
            /// <summary>
            /// Keeps track of the cells that should be updated.
            /// </summary>
            /// <param name="args"></param>
            public void SetUpdatedCells( Dictionary< string, string> args)
            {
            ...
            /// <summary>
            /// Computes the maximum row being displayed. Maximum row is based on the height of the window, the current first row,
            /// and heights of the rows.
            /// </summary>
            /// <returns></returns>
            public double maximumRow()
            {
                double displayed = (this.Size.Height / DATA_ROW_HEIGHT) - 2;
                return _firstRow + displayed;
            }

            /// <summary>
            /// Computes the maximum column being displayed. Maximum column is based on the width of the window, the current first column,
            /// and widths of the columns.
            /// </summary>
            /// <returns></returns>
            public double maximumCol()
            {
                double displayed = (this.Size.Width / DATA_COL_WIDTH) - 1;
                return _firstColumn + displayed;
            }
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                // Fills in the cells that have been changed as a a result of another cell changing
                if (cellsChanged != null)
                {
                    int col, row;
                    foreach (string cell in cellsChanged.Keys)
                    {
                        // Gets the cell location and updates it to the new value
                        col = cell[0] - 65;
                        row = Int32.Parse(cell.Substring(1)) - 1;

                        Brush colorBrush = new SolidBrush(Color.LightCyan);
                        e.Graphics.FillRectangle(colorBrush, new Rectangle(LABEL_COL_WIDTH + (col - _firstColumn) * DATA_COL_WIDTH + 2,
                                          LABEL_ROW_HEIGHT + (row - _firstRow) * DATA_ROW_HEIGHT + 2,
                                          DATA_COL_WIDTH - 3,
                                          DATA_ROW_HEIGHT - 3));
                    }
                    cellsChanged.Clear();
    ...
    }// End of SpreadsheetPanel

}// End of the SS namespace